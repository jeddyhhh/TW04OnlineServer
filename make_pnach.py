"""Generate the PCSX2 .pnach that redirects TW04's lobby and skips DNAS.
    ...
    python tools/make_pnach.py 192.168.1.50
    python tools/make_pnach.py 192.168.1.50 --port 10200 --out <pcsx2>/cheats
...
def build(ip, port, strong=False, comment=None):
    out = [
        ...
        'comment=%s' % (comment or 'Lobby redirect + DNAS bypass. '
                        'Generated by tools/make_pnach.py.'),
        '',
    ...
    if strong:
        ...
        out += [
            ...
            '',
            '// Stronger version, stops DNAS running at all -- tools/make_pnach.py --strong-dnas',
        ]
        for n, (word, note) in enumerate(DNAS_DRIVER_NEW):
            out.append('//' + emit(DNAS_DRIVER + n * 4, word, note))
    out += [
...
def main():
    ...
    ap.add_argument('--out', default=None,
                    help="PCSX2 cheats directory (default: alongside this repo's notes/)")
    args = ap.parse_args()
    ...
    outdir = args.out or os.path.join(os.path.dirname(os.path.dirname(
